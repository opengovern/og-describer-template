...
func ListEnvironments(ctx context.Context, handler *provider.RenderAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
	var wg sync.WaitGroup
	renderChan := make(chan models.Resource)
	errorChan := make(chan error, 1) // Buffered channel to capture errors
	projects, err := provider.GetProjects(ctx, handler)
	if err != nil {
		return nil, err
	}
...
func GetEnvironment(ctx context.Context, handler *provider.RenderAPIHandler, resourceID string) (*models.Resource, error) {
	...
	value := models.Resource{
		...
		Description: JSONAllFieldsMarshaller{
			Value: provider.EnvironmentDescription{
				ID:              environment.ID,
				Name:            environment.Name,
...
func processEnvironments(ctx context.Context, handler *provider.RenderAPIHandler, projectID string, renderChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var environments []provider.EnvironmentJSON
	var environmentListResponse []provider.EnvironmentResponse
...
func processEnvironment(ctx context.Context, handler *provider.RenderAPIHandler, resourceID string) (*provider.EnvironmentJSON, error) {
	var environment provider.EnvironmentJSON
	var resp *http.Response
	baseURL := "https://api.render.com/v1/environments/"
