...
func ListJobs(ctx context.Context, handler *provider.RenderAPIHandler, stream *models.StreamSender) ([]models.Resource, error) {
...
func processJobs(ctx context.Context, handler *provider.RenderAPIHandler, serviceID string, renderChan chan<- models.Resource, wg *sync.WaitGroup) error {
	var jobs []provider.JobJSON
	var jobListResponse []provider.JobResponse
	...
	for _, job := range jobs {
		wg.Add(1)
		go func(job provider.JobJSON) {
			defer wg.Done()
			value := models.Resource{
				ID:   job.ID,
				Name: job.Status,
				Description: JSONAllFieldsMarshaller{
					Value: provider.JobDescription{
						ID:           job.ID,
						ServiceID:    serviceID,
