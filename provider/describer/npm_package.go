...
func GetNPMPackage(ctx context.Context, githubClient GitHubClient, organizationName string, repositoryName string, resourceID string, stream *models.StreamSender) (*models.Resource, error) {
	var packageData model.PackageResponse
	baseURL := "https://api.github.com/orgs/"
	client := http.DefaultClient
	finalURL := fmt.Sprintf("%s%s/packages/%s/%s", baseURL, organizationName, "npm", resourceID)
	req, err := http.NewRequest("GET", finalURL, nil)
	if err != nil {
		return nil, err
	}
