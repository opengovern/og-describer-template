...
func GetRubyGemsPackageList(ctx context.Context, githubClient GitHubClient, organizationName string, stream *models.StreamSender) ([]models.Resource, error) {
	...
	for pagesRemaining {
		params := url.Values{}
		params.Set("package_type", "rubygems")
...
func GetRubyGemsPackage(ctx context.Context, githubClient GitHubClient, organizationName string, repositoryName string, resourceID string, stream *models.StreamSender) (*models.Resource, error) {
	var packageData model.PackageResponse
	baseURL := "https://api.github.com/orgs/"
	client := http.DefaultClient
	finalURL := fmt.Sprintf("%s%s/packages/%s/%s", baseURL, organizationName, "rubygems", resourceID)
	req, err := http.NewRequest("GET", finalURL, nil)
	if err != nil {
		return nil, err
	}
